...
if($step != '1')
{
    $result = mysqli_query($mysql_link, "SELECT * FROM `hosts` where `id`=''" )
...
else
{
  ...
  $result = mysqli_query($mysql_link, "SELECT * FROM `hosts` where `id_group` LIKE '$serialsearch' ORDER BY `name`" )
                         or die (mysqli_error($mysql_link)."<br>Couldn't execute query: $query");

...
else
{
   ...
   // this is a new host
      ...
      $query = mysqli_query($mysql_link, "INSERT INTO `hosts` (`name`,`ip`,`id_group`,`serialno`,`memory`,`osversion`,`cabinet`,`uloc`,`cageno`,`model`,`procno`,`provider`,`install_dt`,`po`,`cost`,`maint_cost`,`maint_po`,`maint_provider`,`maint_end_dt`,`life_end_dt`,`ostype`,`osvers`,`intf1`,`intf2`,`defaultgw`,`monitor`,`selinux`,`datechgroot`) VALUES('$name','$ip','$group','$serialno','$memory','$osversion','$cabinet','$uloc','$cageno','$model','$procno','$provider','$install_dt','$po','$cost','$maint_cost','$maint_po','$maint_provider','$maint_end_dt','$life_end_dt','$ostype','$osvers','$intf1','$intf2','$defaultgw','$monitor','$selinux','$datechgroot')" ) or die(mysqli_error($mysql_link)."<br>Couldn't execute query: $query");
      $id = mysqli_insert_id();
